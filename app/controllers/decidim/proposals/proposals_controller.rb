    ...
    class ProposalsController < Decidim::Proposals::ApplicationController
      ...
      def publish
        ...
        PublishProposal.call(@proposal, current_user) do
          on(:ok) do
            link = Rails.application.config.external_links[:draw_contact]
            flash[:notice] = I18n.t("proposals.publish.success", scope: "decidim", link: link)
      ...
      def update
        ...
        UpdateProposal.call(@form, current_user, @proposal) do
          on(:ok) do |proposal|
            link = Rails.application.config.external_links[:draw_contact]
            flash[:notice] = I18n.t("proposals.update.success", scope: "decidim", link: link)
            flash[:html_safe] = true
