...
class Comment extends React.Component<CommentProps, CommentState> {
          ...
          <div className="author-data">
            ...
            <div className="author-data__extra">
              <button type="button" title={I18n.t("components.comment.report.title")} data-open={modalName}>
                ...
                <span className="author-data__extra-title">
  ...
  private _renderReplyButton() {
    const { isRootComment, comment: { acceptsNewComments }, session } = this.props;

    if (isRootComment && session && acceptsNewComments) {
      return (
        <button
    ...
    if (session && showReplyForm) {
      return (
        <AddCommentForm
          ...
          commentable={comment}
          showTitle={false}
          submitButtonClassName="button small secondary hollow"
                ...
                (
                  <form key={`report-form-comment-${id}`} method="post" action={`/report?sgid=${sgid}`}>
                    ...
                    <div className="text-center">
                      <button type="submit" name="commit" className="button secondary hollow large">{I18n.t("components.comment.report.action")}</button>
                    </div>
                  </form>
                )
              ];
